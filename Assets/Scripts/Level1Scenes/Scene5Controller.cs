...
public class Scene5Controller : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Mouse.current != null && Mouse.current.leftButton.wasPressedThisFrame)
        {
            ...
            else if (stepState == StepState.Idle && currentStepIndex < dialogueSteps.Count && dialogueSteps[currentStepIndex].stepType == StepType.DialogueOnly)
            {
                currentStepIndex++;
                ShowCurrentStep();
            }
    ...
    void EndScene()
    {
        StartCoroutine(FadeOutAndGoToScene6());
    }

    IEnumerator FadeOutAndGoToScene6()
    {
        float t = 0f;
        while (t < dialogueFadeDuration)
        {
