...
public class Scene42Controller : MonoBehaviour
{
    [Header("UI")]
    ...
    [Header("Scene To Load (for Single Button)")]
    public string sceneToLoad = "MainMenuPart3"; 
    ...
    void ShowSingleButton()
    {
        ...
        singleContinueButton.onClick.AddListener(LoadMainMenuScene);
    }

    void LoadMainMenuScene()
    {
        // Play sound if SoundManager exists and sounds are enabled
        if (SoundManager.Instance != null && SoundManager.Instance.sfxSource != null)
        {
            SoundManager.Instance.PlayPlayButton(); // optional click sound
        }
